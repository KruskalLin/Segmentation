...
def load_weights_sequential(target, source_state):
    ...
    print("load weights sequential ...")
    # print([k for k,v in model_to_load.items()])
...
def resnet18(pretrained=True):
    ...
    if pretrained:
        load_weights_sequential(model, torch.load(model_urls['resnet18']))
    return model
...
def resnet34(pretrained=True):
    ...
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet34'], model_dir="./resnet"))
    return model
...
def resnet152(pretrained=True):
    ...
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet152'], model_dir="./resnet"))
    return model
